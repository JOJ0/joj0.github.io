...
![pic 3](/images/2018-5-10-volca-beats-m4l-jtctrl/Volca_Beats_JTCTRL_v0.8.3.png)
...
* The "Blocked" button is a weird one. It changes the "default loading behaviour" of Ableton Live projects containing M4L devices.
    * Normally whenever you are loading a Live project, all contained M4L devices "bang" out their settings of their dials via MIDI - which means: Whenever you are loading a Live project, immediately your Volca Beats sounds different - I don't like this behaviour, so this M4L device prevents the data to be sent to the Volca Beats - it does not let MIDI CC's through as along as you _manually_ switch _off_ the "Blocked" button!
    * So basicaly the user has to first unblock the device with this switch before it can be used!

* And yes, certainly JTCTRL makes your Volca Beats velocity sensitive, when it's controlled via MIDI notes, exactely like the Velocitizer does.

The device is tried and tested, I am using it on each and every gig with my band ADHS - It's stable! I am up for feature requests!
